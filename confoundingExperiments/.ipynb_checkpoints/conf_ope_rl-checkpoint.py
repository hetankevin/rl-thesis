...
def simulate_rollouts( nS, nA, P, Pi, state_dist, n ):
    ...
    for x in range(n):
        a = np.arange(0, nA)[np.searchsorted(np.cumsum(Pi[:,currentState]), np.random.rand(1))]
        ...
        nextState = np.arange(0, nS)[np.searchsorted(np.cumsum(currentRow), np.random.rand(1))]
